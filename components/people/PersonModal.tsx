...
import { Person, Shop } from '@/lib/types';
...
export function PersonModal({ person, isOpen, onClose, onShopSelect }: PersonModalProps) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const { opacity: stickyHeaderOpacity, resetOpacity } = useStickyHeaderOpacity(scrollRef);

  // Reset scroll position when person changes
  useEffect(() => {
    if (person) {
      resetOpacity();
    }
  }, [person?.documentId, resetOpacity]);

  if (!person) return null;

  const photoUrl = getMediaUrl(person.photo);
  const picks = person.person_picks ?? [];
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  ...
                  {person.youtube && (
                    <a
                      href={person.youtube.startsWith('http') ? person.youtube : `https://youtube.com/${person.youtube}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-sm text-text-secondary hover:text-primary transition-colors"
                    >
