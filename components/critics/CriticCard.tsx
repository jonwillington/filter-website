...
export function CriticCard({ critic, onClick, variant = 'default' }: CriticCardProps) {
  const isLight = variant === 'light';
  const photoUrl = getMediaUrl(critic.photo);
  const picksCount = critic.critic_picks?.length ?? 0;

  // Truncate bio for preview
  const bioPreview = critic.bio
    ? critic.bio.length > 100
      ? critic.bio.slice(0, 100).trim() + '...'
      : critic.bio
    : null;
